...
const (
    ...
    // DefaultTLSTimeout is the default amount of time an Attacker waits for a tls handshake
    // before it times out.
...
// TLSTimeout returns a functional option which sets the TLSHandshakeTimeout of time
// an Attacker will wait for a tls hand shake
func TLSTimeout(d time.Duration) func(*Attacker) {
    return func(a *Attacker) {
        tr := a.client.Transport.(*http.Transport)
